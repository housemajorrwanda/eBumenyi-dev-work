...
export default function CreateCommunityScreen() {
  ...
  const handleCreateCommunity = async () => {
    if (!communityName.trim()) {
      alert('shyiramo izina rya kominote');
      return;
    }
    ...
    try {
      ...
      // Upload photo if selected (only if communityPhoto is a valid URI)
      if (communityPhoto && communityPhoto.startsWith('file://')) {
        try {
      ...
      if (conversation?.id) {
        ...
        setIsCreating(false);
        alert('Gufungura kominote byanze');
      }
    } catch (error: any) {
      console.error('Error creating community:', error);
      setIsCreating(false);
      alert(
        'Gufungura kominote byanze: ' +
  ...
  return (
    <SafeAreaView style={styles.container} edges={['bottom']}>
      <View style={[styles.header, { paddingTop: insets.top + 8 }]}>
        ...
        <Text style={styles.headerTitle}>Andika kominote</Text>
        <View style={{ width: 20 }} />
      </View>
